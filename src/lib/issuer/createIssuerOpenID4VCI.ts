...
import { CredentialOfferCreateSuccess, IssueCredentialResponse, IssuerOpenID4VCI, PlainIssueCredentialResponse } from "./IssuerOpenID4VCI";
import { GenericStore } from "../core/Store";
import { State } from "./State";
import { MemoryStore } from "../core/MemoryStore";
import { FindAccount } from "./Account/FindAccount";
import { CredentialOffer, CredentialOfferSchema, OpenidCredentialIssuerMetadata, OpenidCredentialIssuerMetadataSchema } from "wallet-common";
...
export function createIssuerOpenID4VCI(url: string, credentialIssuerCreateOptions: CredentialIssuerCreateOptions): IssuerOpenID4VCI {
	...
	return {
		...
		issueCredential: async (issueCredentialOpts): Promise<IssueCredentialResponse> => {
			...
			const { credential_configuration_id } = issueCredentialOptions.request.data;
			...
			const account = await credentialIssuerCreateOptions.findAccount({
				method: 'POST',
				url: "/credential",
				openidvci: { client: { cliendId: client_id, }, },
			}, sub, "");
			...
			if (issueCredentialOptions.request.data.proofs === undefined &&
			...
			if (issueCredentialOptions.request.data.proofs !== undefined) {
				...
				const claims = account?.claims ? await account?.claims("issue", scope) : null;
				if (claims === null) {
